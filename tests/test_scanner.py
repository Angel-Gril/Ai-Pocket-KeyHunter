...
def _scan_mocks(monkeypatch, tmp_path, *, fofa_hits, shodan_hits):
    ...
    monkeypatch.setattr("aipocket.scanner.FofaClient", lambda: _make_mock_client(fofa_hits))
    monkeypatch.setattr(
        "aipocket.shodan_client.ShodanClient", lambda: _make_mock_client(shodan_hits)
    )
...
async def test_dedup_second_run_skips_cached_valid_credential(tmp_path, monkeypatch):
    ...
    from aipocket.dedup import RedisDedupStore

    store = RedisDedupStore(fakeredis.FakeAsyncRedis(decode_responses=True))
    monkeypatch.setattr("aipocket.scanner.get_dedup_store", lambda: _returning(store))
    ...
    monkeypatch.setattr("aipocket.scanner.validate_all", counting_validate)
